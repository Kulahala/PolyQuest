...
- For an approved PolyQuest stage, Main leads and implements the scoped non-GAS and GAS source/config/documentation work, plus permitted Editor authoring. After each coherent GAS change set, Main must teach the result instead of merely reporting it: explain the runtime problem, file/class ownership, lifecycle timing, data and Gameplay Tag contracts, the important code paths, why the chosen design fits GAS, deliberate non-goals, and the exact static/Editor/compile/PIE checks still required. User-led hands-on exercises remain available when the user explicitly asks to write a specific slice, but they are no longer the default.
...
- PolyQuest is a GAS learning project. The default mode is Main-led implementation with structured teaching after the change. Explain a coherent feature slice as a connected system rather than scattering instructions one file or one line at a time. Every GAS implementation explanation must cover: (1) the player/runtime problem being solved, (2) ownership and why the responsibility is placed in ASC, Ability, GameplayEffect, GameplayTag, AttributeSet, AbilityTask, AnimNotify, Character, or Blueprint, (3) the runtime call chain and when each part becomes active, (4) the key code/data contracts and failure paths, (5) why this is the selected GAS design and what alternatives were rejected, (6) what the slice intentionally does not implement, and (7) a focused verification checklist. When useful, explain the files in a stable order—contract/header, implementation/lifecycle, data assets, then integration and validation—but do not force a one-file-at-a-time handoff. User-led hands-on exercises remain supported for explicitly requested practice slices. After implementation, use MCP and source tools for read-only confirmation; do not present static inspection as compile or PIE evidence.
- Keep the teaching cadence incremental. Split the explanation by one implementation idea or one runtime-link segment at a time—for example ASC initialization, input-to-ability activation, CommitAbility and Cost, AbilityTask and GameplayEvent, Trace and target Effect, then cleanup. After explaining one segment, stop and wait for the user's confirmation or question before continuing. Do not dump the entire feature tutorial into one response, even when the implementation itself was completed as one coherent change set.
...
- Before requesting a user compile for a non-trivial C++ change, perform lightweight static review, `git diff --check`, and a targeted `server-memory` query for the touched Unreal/GAS error family. When a compile or runtime failure is confirmed as reusable, write a concise entity through `server-memory` after the fix and read it back; store the trigger, wrong approach, correct fix, scope, and verification, not raw transient logs. For later source changes, query the same error family again and check the diff/static evidence for recurrence before requesting compilation. Use a stable `MEMORY_FILE_PATH`; do not rely on the npm package directory as durable storage. If `server-memory` is unavailable, say so explicitly and do not claim the memory operation occurred.
