...
- **编辑可审查性**：文件修改默认使用 Edit / `apply_patch`，便于用户直接查看变更；仅在补丁方式不适合、失败，或脚本有明确效率收益时使用脚本写入。脚本写入前说明理由，写入后核验限定路径 diff，避免无关替换、编码或换行变化。
...
- **`README.md`**：只负责项目介绍、演示、主要玩法、技术特点、运行条件与文档导航。仅在对外能力或使用条件变化时更新，不追加任务编号、阶段流水账或逐项验证收据。
- **`ARCHITECTURE.md`**：记录**当前代码库已验证的稳定事实（Current Truth）**。严禁作为待办或过程日志；仅在功能完成并经过验证和审查后更新。
- **`ROADMAP.md`**：记录**未来里程碑路线、依赖顺序与已知技术债（Future Route）**。仅列开放阶段；已完成阶段归档后移除详细完成块，顶部保留最近交付和当前任务指针，更早阶段只保留影响开放任务的必要依赖引用。未关闭债务不得随阶段归档移走。
- **`plan.md`**：记录**当前活跃切片的法定交接凭据（Active Slice Handoff）**。下一切片启动前可保留最近交接；替换时先确认上一阶段核心结果已归档，同步 `ROADMAP.md` 当前入口，并将旧阶段引用固定到提交版本或归档条目，避免历史内容继续指向当前 `plan.md`。
- **`ROADMAP-archive.md`**：历史归档库，仅作为非权威的历史追溯凭证，严禁用于推断当前系统运行时行为。只追加值得追溯且尚未保存的核心结果、证据边界和固定引用；已有归档不重复搬运，不整份复制旧 `README.md` 或 `ROADMAP.md`。
