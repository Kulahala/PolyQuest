...
- **Fresh Review 范围与深度预算：** 默认审查当前批准的修改文件，以及从受影响符号向外展开一层的直接 callers/callees；可只读查看一跳内直接关联的 Tag、Config、测试和资产契约。没有缺陷证据时禁止进行多跳递归漫游。若发现 P0/P1 迹象，或变更涉及公共 API、异步回调、生命周期边界、Gameplay Tag/Input/资产契约，允许说明理由后最小化扩展范围；扩展只用于确认风险，不扩大修改范围。
- **验证证据短路：** 对本次明确记录且确实覆盖目标场景的 Rider、编译、Automation、Editor readback 和用户 PIE 结果直接采纳，不重复运行或逐分支复述已覆盖路径。上述证据不替代对未覆盖的取消、销毁、重入、空安全、契约和回归风险的检查；测试数量使用当前实际记录，不写死为某个固定数字。
- **审查重点与输出预算：** 优先检查与当前变更相关的生命周期/异步清理、空指针与弱引用防御、以及实现和 `plan.md` 冻结契约的一致性；随后做一次短的变更行为与调用边界回归扫查。无阻塞缺陷时直接给出结论、残余观察项和未验证项，不输出重复实现摘要或过程流水账。
