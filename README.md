...
`TODO-02B1` completes the local screen-space lock-on baseline for the fixed oblique camera: Middle Mouse acquires the strictly in-viewport hostile nearest to the cursor or clears the current lock, while the mouse wheel cycles the current lock in deterministic screen-space order. A valid lock highlights the Enemy's existing overhead Health bar and supplies only one-shot facing to Light, Charged, Sprint Attack, and Melee Skill; directional Dodge remains camera-relative and no-input Dodge uses the lock. It does not recenter the camera, continuously rotate the Player, auto-retarget, or change Bow pointer-facing, Release target assist, Homing, collision, or projectile lifetime. The user confirmed `PolyQuest.Player.LockOn` Automation, manual compilation, Editor readback, and focused PIE including the Bow regression. Input/Widget assets remain local `Content/**` WIP; `TODO-02B2` owns continuous locked locomotion facing and death-only retargeting, while `TODO-03B-3` alone owns a Bow lock-target preference.
...
`TODO-02B1` 已完成固定斜视相机下的本地屏幕空间锁定基线：中键获取严格位于视口内、离鼠标最近的敌对目标或清除当前锁定，滚轮则按确定性的屏幕空间顺序切换。有效锁定只高亮 Enemy 既有头顶血条，并为 Light、Charged、Sprint Attack 与 Melee Skill 提供一次性起手朝向；有方向输入的 Dodge 仍保持相机相对，无输入 Dodge 才使用锁定朝向。它不会重置相机、持续旋转 Player、自动换目标，也不改变 Bow 的鼠标指向、Release Target Assist、Homing、碰撞或投射物生命周期。用户已确认 `PolyQuest.Player.LockOn` Automation、手动编译、Editor 读回与包含 Bow 回归的聚焦 PIE。Input/Widget 资产继续作为本地 `Content/**` WIP；持续锁定移动朝向与仅死亡自动换目标归 `TODO-02B2`，Bow 的锁定目标优先仅归 `TODO-03B-3`。
