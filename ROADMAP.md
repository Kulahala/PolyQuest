...
Implemented/PIE-confirmed: TODO-03A3E → TODO-07B4 → TODO-03A7 (entry 0 only) → TODO-05A → TODO-05B → TODO-07B8-A → TODO-05A1-A → TODO-05A1-B → TODO-05A1-C → TODO-05A1-D1
...
Next execution slice: TODO-05A1-D2 (Weapon-Specific Execution Presentation v1)
Next independent enemy route: TODO-03C (Ranged Enemy v1)
Recommended post-C execution-presentation sequence: TODO-05A1-D1 → TODO-05A1-D2 → TODO-05A1-E; then evaluate TODO-07B8-B and the conditional TODO-07B8-C from real authored consumers. D2/E are the current priority because they finish the already-delivered execution loop; TODO-03C remains a separate route, not a hard dependency of those presentation stages.
...
- `TODO-03I2` is closed as the bounded cross-weapon Tag taxonomy migration. Four orthogonal capability/teardown tags now separate Dodge, Defense, Reaction, and UnPossess cancellation semantics; concrete `Ability.Skill.Whirlwind` identity remains authored independently; the legacy `Ability.Skill.Melee` tag is no longer registered. The stage preserved physical inputs, Bow/Charged phase events, Motion-Warp, Trace/Resolver/Damage, and GAS ownership. User-confirmed Automation and Scene01 PIE passed; the headless Dodge end-to-end fixture and independent compile/readback receipts remain validation debt. `TODO-05A`, `TODO-05B`, `TODO-07B8-A`, and `TODO-05A1-A/B/C/D1` have since closed their own source and user gates; `TODO-05A1-D2/E` now take priority for execution presentation, while `TODO-03C` remains an independent route.
...
- `TODO-05A1: Execution Lock-in & Recovery v1` 已完成。A/B/C/D1 四个切片依次闭合成对锁定、授权命中/DeathPending、Authenticated Release、致死/非致死结果分流与通知时序驱动的受害者表现；稳定契约和验证证据见 `plan.md` 与 `ROADMAP-archive.md`。D2/E 是当前优先的表现闭环，`TODO-03C` 仍保留为独立的远程敌人路线，不在本条重复展开已完成切片。
...
- [ ] TODO-05A1-D2: Weapon-Specific Execution Presentation v1
  - 在 `UMeleeWeaponDefinition` 增加窄的 Front/Backstab Player Execution Montage 配置；激活时从当前主手近战武器读取并快照，Hit、Release、Motion Warp 和 Notify 校验只使用本次会话快照。
  - 处决锁定期间拒绝装备切换，防止武器网格、武器风格和 Montage 中途错配；缺少武器专属 Montage 时在 Commit 前 fail-closed 回落普通攻击，不静默使用全局 Ability CDO Montage。
  - 沿用现有 Hit/Release Notify、距离和双人锁定契约；`VictimStart` 时序由 D1 负责，不复制 C++ Ability 子类，不让武器资产直接引用敌方骨架动画。只有真实配对资产证明需要风格映射时，才另行评估窄的 style key。需要真实武器 Montage 作者化、Editor readback、Automation、编译和 Scene01 PIE。
...
- `TODO-07B8` remains an accepted umbrella for concrete Enemy Montage timing needs. Its Slice A (`TODO-07B8-A`) now has the opt-in Stance Break implementation, focused Automation, user PIE, and Main review closeout; `TODO-07B8-B` gives future HitReaction/Launch/Melee consumers individual assets-and-tests gates, while `TODO-07B8-C` is the conditional Player/Enemy helper pilot rather than a batch rewrite. `TODO-05A1-A/B/C/D1` has now closed the paired lock, lethal-hit Recovery, Release outcome, and notification-timed victim presentation slices; D2/E are the prioritized execution-presentation follow-ups. Keep the RateWindow and execution routes independently gated; a shared execution base may still be considered only after measured 05A/05B duplication and a separately accepted contract. These follow-ups remain separate from the `TODO-03C` ranged-enemy contract.
...
- `REC-05A1-03`: Unified Execution Hit Notify and Tag (conditional recommendation). The current Front and Backstab Notify classes have the same payload shape and their Abilities are mutually exclusive, so they may converge on one fixed `UAnimNotify_PlayerExecutionHit` that emits the exact `Event.Action.Execution.Hit` tag. Activate only when a curated inventory of active Front/Backstab (including any weapon-specific) Montages is available and the authoring benefit is demonstrated. The migration must preserve separate Front/Backstab Ability ownership and validation of execution type, Context, activation token, Montage/Sequence identity, target geometry, and exactly-once state; a common Notify must not bypass those checks or accept a broad parent/child event by analogy. Replace Notify instances through the Unreal Editor, read back timing and references, run both focused Automation suites plus user compile and Scene01 PIE, and only then remove the old Notify classes/tags after zero-reference evidence. Do not patch `.uasset` files or perform an automatic asset migration. If the asset inventory or readback is incomplete, retain the existing two Notify/tag paths and record no-adoption rather than risking silent Montage failure. This recommendation is an authoring-contract cleanup, separate from the D1 `VictimStart` timing contract and from any future Boss grab/throw requirement.
