...
- The source implementation closeouts for `TODO-03A7B` (Light entry `0..2`), `TODO-03A7C` (Charged release/Sprint Attack), and `TODO-03A7D` (Melee Skill) are recorded in `plan.md` and `ROADMAP-archive.md`; their authored Montage/Notify/Root Motion readback and separate manual compile records remain non-blocking validation debt. `TODO-03A7F` has now closed the shared trigger-range source contract; its authored migration/readback debt is tracked below. The next open review slice is `TODO-03H5`.
...
- [ ] TODO-03H5: Post-Motion-Warp Combat Health Review v1
  ...
  - Review the final `TODO-03A7F` trigger-range/reverse-correction contract as part of the same health gate; do not reopen completed adoption slices merely to retune authored values.
...
- [ ] TODO-03I1: Unified Combat Input Contract And Loadout Simplification v1
  - Audit the common physical controls (`Primary`, `Guard`, `Parry`, and prepared slots) across current melee and Bow routes before changing any public or authored contract. The identical keys are a UX observation, not proof that the underlying Ability lifecycles or payload semantics are interchangeable.
  - The current source evidence must be preserved in the plan: `UCombatLoadoutDefinition` still supplies the MainHand `Primary`/`Sprint` route, startup Loadout state, and equipment-transaction rollback identity; `DefenseProfile` resolves Guard/Parry, while `UWeaponDefinition::DefaultPreparedActions` and exact handles resolve `1-4`. Any removal or flattening of `AssociatedLoadout` is conditional on an explicit route-equivalence and asset-migration decision, not an assumed cleanup.
  - Evaluate the smallest stable target: direct weapon fields, a reduced input profile, or retaining the existing Loadout when it remains the correct owner. Preserve `APlayerCharacter` as the physical-input/event publisher, `UWeaponEquipmentComponent` as the single resolver, GAS as activation authority, exact prepared handles, Primary short/hold arbitration, and equipment rollback/teardown semantics.
  - Treat Tag work here as an audit and decision input for `TODO-03I2`, not as a broad rename implementation. Record which tags are input intents, Ability identities, cancellation capabilities, active-window states, semantic phase events, or teardown selectors. `Event.Attack.Bow.DrawReady`/`Release`, Charged HoldReady/ReleaseHandoff, and Skill-specific tags may be candidates only after payload, montage identity, cancellation, and lifecycle equivalence is demonstrated; do not merge distinct event meanings by analogy.
  - Success requires a before/after route matrix for current melee, Bow, Guard/Parry, and `1-4`, no duplicate activation or stale event delivery, preserved equipment rollback and Ability ownership, focused Automation, user-owned compile, curated Editor readback, and Scene01 PIE. No current Mage implementation may be invented as evidence; future Mage support is a compatibility constraint to validate when an authored route exists.
  - No new physical input actions, generic Ability dispatcher/framework, alternate GAS authority, Tag rename/migration, Bow/projectile behavior change, Defense/Trace/Damage change, or blind deletion/migration of Loadout assets in this stage.
