...
- The product route is `/Game/Maps/Scene01` with a fixed elevated oblique camera and a desktop Controller/input boundary. `TODO-03A7B` has an implementation closeout with user-confirmed focused Automation and Scene01 PIE; no separate manual `PolyQuestEditor` compile or Motion-Warp Editor-readback record is available, so its authored-baseline validation debt remains open. `TODO-03A7C` has a source implementation closeout for Charged release and Sprint Attack Motion-Warp adoption with the same user-confirmed focused Automation and Scene01 PIE gates; its authored GA/Montage/Notify/Root Motion readback and separate manual compile record remain open. `TODO-03A7D` has a source implementation closeout for Melee Skill Motion-Warp, while the Whirlwind authored readback/adoption gate remains open. `TODO-03A7F` has a source contract closeout for the explicit trigger interval and user-confirmed focused Automation/Scene01 PIE; authored field migration, Editor readback, and an independent manual compile receipt remain open. `TODO-03H5` has completed its Review-Only cross-family health audit at the current source baseline with no P0-P2 blocker or Source change; its exact-stop P3 and authored validation debts remain open. `TODO-03I1` has closed the Primary/Sprint direct-route and Loadout compatibility-mirror source slice with user-confirmed focused Automation and Scene01 PIE; the user has since confirmed the product Loadout assets were cleared to zero references and the legacy asset was deleted, while the C++ compatibility mirror remains for `TODO-03I4`. `TODO-03I2` has now closed the cross-weapon cancellation/tag taxonomy migration with user-confirmed Automation and Scene01 PIE; its Dodge end-to-end unit-test fixture and independent compile/readback records remain open. `TODO-07B5` has now closed the Small Hit Reaction retrigger source slice with user-confirmed focused Automation and Scene01 PIE; its real ASC cross-frame retrigger fixture and independent manual compile/readback receipts remain open as validation debt. `TODO-07B6` has closed the Combat Feedback DataAsset consolidation source slice with user-confirmed profile configuration, Automation, and Scene01 PIE; its profile-null coverage, diagnostic symmetry, and independent manual compile receipt remain open as non-blocking debt. The next planning pointer is `TODO-03I4`, followed by the accepted `TODO-07B7` profile-taxonomy stage; detailed stage records remain in `plan.md`.
...
Current implementation closeouts: TODO-03A7B (Light entry 0..2 lifecycle adoption) → TODO-03A7C (Charged/Sprint source adoption; authored compile/readback debt remains) → TODO-03A7D (Melee Skill source adoption; Whirlwind authored readback debt remains) → TODO-03A7F (explicit trigger-range contract; authored migration/readback debt remains) → TODO-03H5 (Review-Only health audit; no P0-P2 blocker) → TODO-03I1 (Primary/Sprint direct route and Loadout compatibility mirror; product assets now confirmed zero-reference) → TODO-03I2 (cross-weapon cancellation/tag taxonomy; Dodge E2E fixture debt remains) → TODO-07B5 (Small Hit Reaction retrigger; real ASC cross-frame fixture debt remains) → TODO-07B6 (Combat Feedback DataAsset consolidation; profile-null/compile debt remains)
Next player-combat slices: TODO-03I4 → TODO-07B7 → TODO-05A → TODO-05B → TODO-03C
...
`TODO-03A7` remains deliberately limited to the selected Light entry 0; `TODO-03A7B` owns the bounded Light entry 0..2 implementation and lifecycle bridge, while authored compile/readback closure remains a separate validation debt. `TODO-03A7C` owns the bounded Charged release and Sprint Attack source lifecycles, reusing the narrow evaluator/Player bridge without creating a global target-follow service; its authored opt-in/readback debt remains separate. `TODO-03A7D` owns the bounded `UPlayerMeleeSkillAbility` source lifecycle and its independent static snapshot; the actual `GA_Skill_Whirlwind` adoption decision still requires its own readback or evidence-backed no-adoption record. `TODO-03A7F` closes the shared source contract for one explicit target-distance interval (`MinTriggerDistance <= WarpStopDistance <= MaxTriggerDistance`): `MinTriggerDistance < WarpStopDistance` implicitly permits bounded reverse correction, equality preserves forward-only behavior, and the exact-stop point is a no-op. The intentional authored-field migration still requires user readback; old `MaxWarpDistance` values map to `MaxTriggerDistance = WarpStopDistance + old MaxWarpDistance`. `TODO-03H5` has now completed a bounded Review-Only audit of the four Player Motion-Warp consumers and found no P0-P2 blocker or Source change; the exact-stop test limitation and authored validation gaps remain explicit debts. `TODO-03I1` now makes `UWeaponDefinition::PrimaryAttackAbilityTag` and optional `SprintAttackAbilityTag` the sole canonical MainHand Primary/Sprint route, while `AssociatedLoadout` and `ActiveCombatLoadout` remain migration compatibility mirrors that cannot override routing or fail an equipment transaction. The user has confirmed the product Loadout assets were cleared to zero references and the old Loadout asset was deleted; `TODO-03I4` is the separate source/test compatibility-removal gate. `TODO-03I2` has completed the bounded cross-weapon Tag taxonomy migration: cancellation and teardown selectors now use orthogonal capability/lifecycle tags, concrete skill identity remains separate, and the legacy `Ability.Skill.Melee` tag is no longer registered. Its remaining Dodge E2E fixture debt is tracked below. Neither stage authorizes a blind all-Tag rename. Later Player adoption slices reuse these narrow primitives only after their own Montage and Root Motion evidence. TODO-07B4 is a deliberately early presentation gate and is technically independent of Motion Warping. The post-I2 player-combat sequence is now `TODO-07B5 → TODO-07B6 → TODO-03I4 → TODO-07B7 → TODO-05A → TODO-05B`; 07B5 is closed at the source/PIE gate with its real ASC cross-frame retrigger fixture debt tracked below, 07B6 consolidates existing feedback data/ownership, I4 removes the now-unreferenced Loadout compatibility surface, and 07B7 splits the role-specific feedback profiles before the punish stages add front execution followed by Backstab. `TODO-03A4B` is conditional equipment work and is not a prerequisite unless a partial-Guard weapon is intentionally authored.
...
- The source implementation closeouts for `TODO-03A7B` (Light entry `0..2`), `TODO-03A7C` (Charged release/Sprint Attack), and `TODO-03A7D` (Melee Skill) are recorded in `plan.md` and `ROADMAP-archive.md`; their authored Montage/Notify/Root Motion readback and separate manual compile records remain non-blocking validation debt. `TODO-03A7F` has closed the shared trigger-range source contract, and `TODO-03H5` has completed its Review-Only health audit with no P0-P2 blocker or Source change. `TODO-03I1` has closed the direct route and Loadout compatibility-mirror source slice; the user has since confirmed zero product Loadout references and deletion of the old asset, while source cleanup is assigned to `TODO-03I4`. `TODO-03I2` has closed the bounded cross-weapon Tag taxonomy migration; its Dodge end-to-end fixture and independent compile/readback debts remain tracked below. `TODO-07B5` is now closed at the source/PIE gate; its real ASC cross-frame retrigger fixture debt is tracked below. `TODO-07B6` has closed the feedback DataAsset source slice with user-confirmed profile configuration, Automation, and Scene01 PIE; its profile-null coverage and independent compile receipt remain tracked below. The next open planning slice is `TODO-03I4`, followed by accepted `TODO-07B7`.
...
- `TODO-03I1` is closed at the source boundary: MainHand `PrimaryAttackAbilityTag` and optional `SprintAttackAbilityTag` are now the canonical Primary/Sprint route, Guard/Parry still resolve through the Effective Defense Profile, and prepared `1-4` still use exact handles. The user has confirmed the product Loadout assets were cleared to zero references and the old Loadout asset was deleted; `AssociatedLoadout`/`ActiveCombatLoadout` remain only in the source/test compatibility surface until `TODO-03I4` removes them. The detailed route matrix, test evidence, and migration boundary are in `plan.md` and the I1 archive entry.

- `TODO-03I2` is closed as the bounded cross-weapon Tag taxonomy migration. Four orthogonal capability/teardown tags now separate Dodge, Defense, Reaction, and UnPossess cancellation semantics; concrete `Ability.Skill.Whirlwind` identity remains authored independently; the legacy `Ability.Skill.Melee` tag is no longer registered. The stage preserved physical inputs, Bow/Charged phase events, Motion-Warp, Trace/Resolver/Damage, and GAS ownership. User-confirmed Automation and Scene01 PIE passed; the headless Dodge end-to-end fixture and independent compile/readback receipts remain validation debt. The next open implementation slice is `TODO-03I4` after the 07B6 feedback closeout.

- `TODO-07B6` is closed as the bounded Combat Feedback DataAsset consolidation. `UCombatFeedbackDataAsset` supplies authored Overlay, tier Camera Shake/hit-stop, impact, received-hit, Guard, and Parry data while `ABaseCharacter`, Player, Enemy, Controller, Guard, and Parry retain their established runtime owners and cleanup. User-confirmed profile configuration, focused Automation, and Scene01 PIE passed; profile-null/diagnostic symmetry and an independent manual compile receipt remain validation debt. The actual profile assets are user-owned WIP and are not implied to be part of the source/document commit.

- [ ] TODO-03I4: Legacy Combat Loadout Compatibility Removal v1
  - Start after the user-confirmed Editor precondition for I1: the user first verified zero product references in Reference Viewer, then cleared the product references and deleted the legacy `DA_Melee_CombatLoadout` asset. This precondition is now reported as satisfied; the stage still requires a fresh source/test audit before deleting the remaining compatibility surface.
  - Remove only the obsolete C++/test compatibility surface (`UCombatLoadoutDefinition`, mirror fields/accessors/sync branches, stale includes and compatibility assertions) while preserving the I1 direct route: `PrimaryAttackAbilityTag`, optional `SprintAttackAbilityTag`, Effective Defense Profile, and exact prepared-slot handles remain the sole runtime sources.
  - Re-run the affected equipment/projectile/transaction Automation, compile the `PolyQuestEditor`, and use Editor/Reference Viewer plus Scene01 PIE to verify Primary/Sprint, Guard/Parry, pickup, rollback, and no-loadout startup behavior. No new input route, ability dispatcher, asset migration, or gameplay behavior is authorized.

### Combat Feedback Authoring

- [ ] TODO-07B7: Combat Feedback Profile Taxonomy Split v1
  - Start after `TODO-03I4` and before `TODO-05A`; the current Player/Enemy profiles already demonstrate a real authoring problem because role-specific fields are mixed in one panel. This stage is not triggered merely by adding another enemy.
  - Keep `ABaseCharacter` as the runtime owner of the common feedback contract, retain Overlay in the common base, and introduce typed Player/Enemy profile subclasses whose panels contain only the fields consumed by that role. Split the mixed tier settings by responsibility rather than hiding the same fields behind more categories.
  - Migrate the existing Player and Enemy feedback assets in Editor, verify class assignment and Reference Viewer/readback, and update transient Automation fixtures for correct-profile, wrong-profile, null-profile, and optional-resource fail-closed behavior. User-owned `.uasset` migration remains outside the source/document commit.
  - Preserve character-owned lifecycle/cleanup, GAS/Damage/Hit-Stop ownership, and existing feedback behavior. No GameplayCue, global dispatcher, generic feedback framework, second damage path, or automatic asset migration is authorized.
  - The single `PolyQuest` runtime module remains sufficient; no `Build.cs` or module split is expected. Any class rename, redirector, or package-path migration must be explicitly listed in the accepted stage plan before implementation.
...
- [ ] TODO-05A: Stagger Front Execution v1
  - Start after `TODO-07B5`, `TODO-07B6`, `TODO-03I4`, and `TODO-07B7` close their focused reaction/feedback, compatibility-cleanup, and profile-authoring gates; this keeps the execution slice focused on target reservation, geometry, Montage, hit, and cleanup rather than absorbing basic combat presentation or legacy removal work.
...
- [ ] TODO-07B: Feedback, Presentation Retune, And Demo Polish v1
  - TODO-07B1/07B1A, TODO-07B2, TODO-07B3, TODO-07B4, TODO-07B5, and TODO-07B6 are delivered; TODO-07B7 is the accepted follow-up profile-taxonomy stage. The umbrella owns focused presentation work, not new combat ownership; legacy Loadout removal is tracked separately under `TODO-03I4`.
...
- For Combat Feedback authoring, `TODO-07B7` is now accepted because the existing Player and Enemy profiles already mix role-exclusive fields and create editor ambiguity. Split only the common Overlay contract from typed Player/Enemy profile data, preserve character-owned runtime lifecycles, and require explicit asset migration/readback. Do not add further per-enemy subclasses unless a later enemy has a genuinely different feedback contract; keep this work separate from `TODO-03I4`.
...
- TODO-03I1 source closeout establishes direct MainHand Primary/Sprint routes and keeps `AssociatedLoadout`/`ActiveCombatLoadout` as compatibility mirrors. User-confirmed focused Automation and Scene01 PIE are recorded; the user first verified zero product Loadout references in Editor Reference Viewer, then cleared the references and deleted the old Loadout asset. The remaining source/test compatibility surface is owned by `TODO-03I4`; no independent manual `PolyQuestEditor` compile receipt is archived for I1.
...
- TODO-07B6 source closeout establishes `UCombatFeedbackDataAsset` as the authored source for Overlay, tier Camera Shake/hit-stop, impact, received-hit, Guard, and Parry feedback while preserving the existing runtime owners. The user confirmed both product profiles were configured in Editor and that the focused feedback Automation and Scene01 PIE passed. `Debt-07B6-ProfileNullCoverage` remains open because the headless matrix does not explicitly drive a null profile through every owner; the diagnostic Warning symmetry for received/Guard/Parry sound is also not identical. Close with a focused null-profile/diagnostic test or an explicit evidence-backed no-adoption decision. No independent manual `PolyQuestEditor` compile receipt is archived for 07B6.
- `TODO-03I4` is now the owning stage for the remaining Loadout compatibility surface. The user first confirmed all product Loadout references were zero in Editor Reference Viewer, then cleared them and deleted the legacy `DA_Melee_CombatLoadout` asset; C++/test references to `UCombatLoadoutDefinition`, `AssociatedLoadout`, `InitialCombatLoadout`, and `ActiveCombatLoadout` still exist and must be removed only through the I4 plan, targeted compile/Automation, and direct-route/PIE regression gates. Do not treat the asset deletion alone as source cleanup.
- `TODO-07B7` is accepted but not yet implemented. Its trigger is the demonstrated role-field ambiguity in the current Player/Enemy profiles, not the count of enemies; close it with typed profile assets, explicit Editor migration/readback, focused Automation, compile, and Scene01 PIE while preserving the existing feedback owners and fail-closed behavior.
