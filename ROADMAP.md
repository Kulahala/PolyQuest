...
- [ ] `TODO-02F: Montage Rate Window And Action Timing v1`
   ...
   - Keep one complete Sequence and one complete Montage per action entry. `Entry` / `Recovery` / `End` remain semantic phases, not speed-control Montage Sections. Do not duplicate or crop the same Sequence solely to obtain a local rate, add a parallel AnimBP action state, or create a global per-tick rate controller.
   - v1 rejects overlapping rate windows. The baseline rate must restore on window end, natural Montage completion, interruption, cancellation, Combo handoff, `EndAbility()`, and EndPlay. Validate that downstream Trace, Hit, Combo, and Cancel timing follows the same animation timeline at the authored rate, while Root Motion remains continuous and no stale rate survives teardown.
