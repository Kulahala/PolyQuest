...
- The product route is `/Game/Maps/Scene01` with a fixed elevated oblique camera and a desktop Controller/input boundary. `TODO-03A7B/C/D/F`, `TODO-03H5`, `TODO-03I1/I2`, `TODO-07B5/B6/B7`, and `TODO-03I4` have implementation closeouts recorded in `ROADMAP-archive.md`; their remaining authored/readback/compile debts are tracked only where still applicable in Known Risks. `TODO-03I4` has removed the zero-reference legacy Loadout C++/test surface, while `TODO-07B7` has split the feedback profile by role. Direct weapon fields, Defense Profile, prepared handles, the equipment transaction, and typed feedback profiles are now the relevant contracts. `TODO-05A` and `TODO-05B` source, focused Automation, user compile/Editor/PIE gates, and Main review are confirmed complete; detailed evidence remains in `plan.md`.
...
Implemented/PIE-confirmed: TODO-03A3E → TODO-07B4 → TODO-03A7 (entry 0 only) → TODO-05A → TODO-05B
...
Next player-combat slice: TODO-03C
...
- TODO-05A/05B follow-up recommendations are deliberately conditional, not current implementation permission: (1) `TODO-05A1` may address bidirectional execution lock-in and lethal-hit Recovery only if later PIE or product acceptance proves the truncation matters; (2) a shared execution base may be considered only after measured 05A/05B duplication and a separately accepted contract; (3) `TODO-07B8` may adopt the existing RateWindow lifecycle for Enemy Stance Break/reaction Montages after real enemy assets exist; (4) `TODO-07B9` may add execution-only Hit-Stop/Camera Shake/audio through the typed feedback profiles after lifecycle semantics stabilize. These follow-ups are not prerequisites for the completed punish slices or `TODO-03C`.
