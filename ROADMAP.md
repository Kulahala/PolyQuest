...
- [ ] `TODO-02D2: Timed Parry And Enemy Poise Counter v1`
  - After D1's Guard and player Guard Break lifecycle are proven, add a Notify-timed front-arc Parry that consumes authored Stamina and routes its success through the existing enemy Poise/stance-break path, not a direct forced-stun shortcut.
  - Define Parry cooldown, false-start, montage-failure, enemy high-Poise, attack-teardown, and player-state cleanup semantics before implementation. Do not add target selection, directional reaction tiers, or a second melee delivery path.
