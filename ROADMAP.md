...
- The product route is `/Game/Maps/Scene01` with a fixed elevated oblique camera and a desktop Controller/input boundary. `TODO-03A7B/C/D/F`, `TODO-03H5`, `TODO-03I1/I2`, `TODO-07B5/B6/B7`, and `TODO-03I4` have implementation closeouts recorded in `ROADMAP-archive.md`; their remaining authored/readback/compile debts are tracked only where still applicable in Known Risks. `TODO-03I4` has removed the zero-reference legacy Loadout C++/test surface, while `TODO-07B7` has split the feedback profile by role. Direct weapon fields, Defense Profile, prepared handles, the equipment transaction, and typed feedback profiles are now the relevant contracts. `TODO-05A`, `TODO-05B`, `TODO-07B8-A`, and `TODO-05A1-A` now have source, focused Automation, user PIE, and Main review closeouts; their separate authored/readback/compile debts remain explicit in Known Risks where applicable. Detailed evidence remains in `plan.md`.
...
- [ ] TODO-05A1: Execution Lock-in & Recovery v1
  ...
  - Slice B, Hit And Lethal Recovery: 保持现有单一路径和 Exactly-Once 结算，增加带当前 execution session 授权的命中；致死命中进入 `DeathPending`，双方继续完成蒙太奇和 Recovery，不能立即触发 `Dead -> CancelAllAbilities -> Ragdoll`。
...
- `TODO-07B8` remains an accepted umbrella for concrete Enemy Montage timing needs. Its Slice A (`TODO-07B8-A`) now has the opt-in Stance Break implementation, focused Automation, user PIE, and Main review closeout; future HitReaction/Launch/Melee consumers still require their own assets and tests. Player RateWindow unification is recorded only as the conditional `TODO-07B8-C` pilot-and-migrate route above, not as an active prerequisite or a batch rewrite. `TODO-05A1-A` has closed the paired lock-in/contact prerequisite, and `TODO-05A1-B` is now the next execution route because product acceptance still requires lethal-hit Recovery. Keep both routes independently gated; a shared execution base may still be considered only after measured 05A/05B duplication and a separately accepted contract. `TODO-07B9` may add execution-only Hit-Stop/Camera Shake/audio through the typed feedback profiles after lifecycle semantics stabilize. These follow-ups remain outside the current `TODO-03C` scope.
...
- `TODO-05A1-A` is closed with source, focused Automation, user compile/PIE, Lock-On retention repair, and Main Fresh Review evidence. The remaining `TODO-05A1-B/C` debt is the current terminal death path (`Dead -> CancelAllAbilities -> Ragdoll`) and the lack of `DeathPending`/delayed execution recovery or explicit nonlethal Launch/Knockback release semantics. Close it only through a separately accepted stage with paired-montage/Release evidence; do not treat the Slice A lock as proof of lethal Recovery.
