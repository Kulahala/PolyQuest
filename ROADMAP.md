...
- The product route is `/Game/Maps/Scene01` with a fixed elevated oblique camera and a desktop Controller/input boundary. `TODO-03A7B/C/D/F`, `TODO-03H5`, `TODO-03I1/I2`, `TODO-07B5/B6/B7`, and `TODO-03I4` have implementation closeouts recorded in `ROADMAP-archive.md`; their remaining authored/readback/compile debts are tracked only where still applicable in Known Risks. `TODO-03I4` has removed the zero-reference legacy Loadout C++/test surface, while `TODO-07B7` has split the feedback profile by role. Direct weapon fields, Defense Profile, prepared handles, the equipment transaction, and typed feedback profiles are now the relevant contracts. `TODO-05A`, `TODO-05B`, `TODO-07B8-A`, and `TODO-05A1-A/B/C/D1` now have source, focused Automation, user validation, and Main review closeouts; `TODO-05A1-D2A` and `TODO-05A1-D2B` now have source, focused Automation, user PIE, and Main review closeouts, with their separate authored/readback/compile debts recorded below. Detailed evidence remains in `plan.md`.
...
- `TODO-03I2` is closed as the bounded cross-weapon Tag taxonomy migration. Four orthogonal capability/teardown tags now separate Dodge, Defense, Reaction, and UnPossess cancellation semantics; concrete `Ability.Skill.Whirlwind` identity remains authored independently; the legacy `Ability.Skill.Melee` tag is no longer registered. The stage preserved physical inputs, Bow/Charged phase events, Motion-Warp, Trace/Resolver/Damage, and GAS ownership. User-confirmed Automation and Scene01 PIE passed; the headless Dodge end-to-end fixture and independent compile/readback receipts remain validation debt. `TODO-05A`, `TODO-05B`, `TODO-07B8-A`, and `TODO-05A1-A/B/C/D1` have since closed their own source and user gates; `REC-05A1-03` Native and MIG authored gates are now closed, `TODO-05A1-D2A/D2B` are closed at their source/Automation/PIE gates, and `TODO-05A1-E` is next, while `TODO-03C` remains an independent route.
...
- `TODO-05A1: Execution Lock-in & Recovery v1` 已完成。A/B/C/D1 四个切片依次闭合成对锁定、授权命中/DeathPending、Authenticated Release、致死/非致死结果分流与通知时序驱动的受害者表现；D2A/D2B 现已分别补齐一次性 Snap 对齐与武器专属 Montage 选择的源码/Automation/PIE 收口，稳定契约和详细证据见 `plan.md` 与 `ROADMAP-archive.md`。`TODO-05A1-E` 仍是当前表现闭环的下一切片，`TODO-03C` 仍保留为独立的远程敌人路线，不在本条重复展开已完成切片。
...
- REC-05A1-03-RET 已完成 Legacy Notify/Tag retirement；强制中断、销毁、UnPossess、Teardown 的 PIE 证据作为已接受的非阻塞验证债务保留在 Known Risks，后续 D2A/D2B 已闭合，当前下一执行切片为 TODO-05A1-E。

- [ ] TODO-05A1-D2: Execution Presentation v1（D2A/D2B 核心主线，D2C/D2D 条件性扩展）
  - [x] TODO-05A1-D2A: Execution Handshake Snap Alignment v1
    - 已完成：握手成功、玩家 Montage 启动前，Front/Backstab 通过同一纯数学 Helper 一次性 Snap 到受害者前/后方；Sweep 临时忽略受害者自身胶囊体，环境阻挡 fail-closed，失败恢复原始 Transform 并清理速度，再沿既有 formal Release 释放受害者。
    - `UMeleeWeaponDefinition::ExecutionSnapDistance` 是武器/动画物理规格；Front/Backstab Ability 的 `MinExecutionDistance`/`MaxExecutionDistance` 仍是玩法触发窗口和安全边界，入口互锁防止 Snap 后命中阶段静默失效。当前不合并字段、不增加 Override。
    - 执行路径不再拥有或清理普通攻击的 Motion-Warp target；用户明确批准删除两个执行 Ability 的旧 Motion-Warp 配置字段。对齐输出保留玩家当前 `Z`，解决已发现的玩家/目标高度差导致背刺下陷问题；复杂坡地/台阶贴地仍是已知限制。
    - 用户确认 `PolyQuest.Combat.ExecutionSnapAlignment`、`FrontExecution`、`Backstab` Automation 和 Scene01 PIE 通过；Main Fresh Review 无 P0-P2 blocker。独立 `PolyQuestEditor` 编译与本阶段执行资产 readback 未形成新收据，仍按 Known Risks 记录。
  - [x] TODO-05A1-D2B: Weapon-Specific Execution Montage Selection v1
    - 已完成：`UMeleeWeaponDefinition` 提供可选 `FrontExecutionMontage`/`BackstabExecutionMontage`；两个 Player Execution Ability 通过同一 resolver 从当前主手读取方向 Montage，并在 Commit 前后锁定武器/Montage 快照。缺失方向 fail-closed，不回退到 Unarmed 或 Ability CDO；既有 `State.Action.Attacking`/`CanSwapNow()` 换装闸门保持不变。
    - Commit 后的 Montage Task、动画身份、D2A Snap 距离与停止逻辑均使用激活快照；Commit 顺序包含测试强制失败、快照有效性、live 身份复核和基类 Commit。既有 VictimStart/Hit/Release、唯一 Resolver、Exactly-Once 与清理合同未改变。
    - 用户确认 Front/Backstab Focused Automation 与 Scene01 PIE 通过；Main Fresh Review 未发现 P0-P2 blocker。Gemini 回交报告列出编译、Rider 和资产 readback，但没有形成 Main 独立归档收据；该 authored/compile debt 继续保留在 Known Risks。
  - [ ] TODO-05A1-D2C: StanceBreak Backstab Compatibility v1（条件性）
    - 只有在产品明确采纳“破防后绕背仍可背刺”后启动。用户已确认当前 StanceBreak 是原地僵直/硬直 Montage，不是跪地或倒地 Montage，因此不把新增跪地分支作为硬前置；首个表现门禁是当前 StanceBreak 与 Backstab Victim Montage 的真实 PIE 组合。
    - 在保持 D2A/D2B 当前拒绝行为的前提下，解除 `PlayerBackstabExecutionAbility` 的 Stunned 门禁和 `EnemyVictimExecutionAbility` 的 active StanceBreak 门禁，但只允许 StanceBreak 所有的单一 `State.Status.Stunned`。必须定义显式 StanceBreak -> Victim handoff：当前 Victim 仅对 Front 设置 `bHandoffFromStanceBreak`，而 StanceBreak 在 `VictimLocked` 时跳过自身移动/Poise 恢复；Backstab 必须补齐 exactly-once 的 Poise/移动恢复所有权，避免释放后残留破韧状态。
    - 允许路径至少包括 `PlayerBackstabExecutionAbility.*`、`EnemyVictimExecutionAbility.*`、必要时 `EnemyStanceBreakAbility.*` 及对应 Automation；覆盖 Front 优先级、握手取消、Montage 中断、阻挡回滚、目标销毁、UnPossess/Teardown、`MOVE_None -> MOVE_Walking`、Poise、AI 锁和 `VictimStart -> Hit -> Release`。不得修改 D2A 的 Snap 几何或引入第二条伤害路径。
  - [ ] TODO-05A1-D2D: Weapon-Authored Execution Range Contract v1（条件性）
    - 只有出现至少两个武器族的真实触发窗口差异、配置重复或维护漂移时启动。将 `ExecutionMinDistance`、`ExecutionMaxDistance` 与 `ExecutionSnapDistance` 放入同一 `UMeleeWeaponDefinition`，默认保持 `0 / 250 / 190 cm`，并明确它们是中心距离/吸附距离还是其他几何量。
    - Ability 读取 DataAsset 作为作者化来源，但保留 Native hard cap、有限性、`Min <= Snap <= Max` 和双入口 fail-closed 校验；完成现有 GA 字段迁移、旧资产 Editor readback、focused Automation、编译与 Scene01 PIE 后才替换当前 Ability-owned 窗口。D2D 与 D2C 独立，不是 D2A 的回补。
...
- `TODO-05A1-D2C`（StanceBreak 后允许 Backstab）是条件性玩法建议，不属于当前 D2A/D2B/E 合同。当前僵直 Montage 已移除原先“跪地姿态穿帮”的先验阻碍，但仍必须先定义 StanceBreak/Victim 的取消、Poise、移动与 AI 锁交接，并重新评估双方 `State.Status.Stunned` 互斥校验；在阶段获批前继续拒绝该组合。
- `TODO-05A1-D2D`（Weapon-Authored Execution Range Contract）是独立的条件性数据驱动扩展。将 Min/Max 与 Snap 放入 Weapon DataAsset 能支持匕首、长剑等不同触发窗口，但必须先有真实武器族证据，并保留 Native hard cap、迁移/readback、默认值和入口 fail-closed 合同；因此不回溯修改 D2A 的分权实现。
...
- D2A/D2B 已完成源码、Focused Automation、用户 Scene01 PIE 与 Main Fresh Review 收口；D2B 的下一执行切片为 `TODO-05A1-E`。D2A/D2B 的独立手动 `PolyQuestEditor (Development Editor)` 编译及执行 Weapon/GA/Montage readback 收据仍未单独归档，继续按非阻塞 authored-validation debt 处理。
