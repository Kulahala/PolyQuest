...
- The product route is `/Game/Maps/Scene01` with a fixed elevated oblique camera and a desktop Controller/input boundary. `TODO-03A7B/C/D/F`, `TODO-03H5`, `TODO-03I1/I2`, `TODO-07B5/B6/B7`, and `TODO-03I4` have implementation closeouts recorded in `ROADMAP-archive.md`; their remaining authored/readback/compile debts are tracked only where still applicable in Known Risks. `TODO-03I4` has removed the zero-reference legacy Loadout C++/test surface, while `TODO-07B7` has split the feedback profile by role. Direct weapon fields, Defense Profile, prepared handles, the equipment transaction, and typed feedback profiles are now the relevant contracts. `TODO-05A` source, Automation, user compile, Editor readback, PIE, and Main review are confirmed complete; detailed evidence remains in `plan.md`.
...
- [ ] TODO-05B: Backstab v1
  ...
  - Before implementation, make a design gate for execution range versus Motion-Warp trigger/stop range. Merge only fields with identical semantics; extract a `PlayerExecutionAbilityBase` only after 05A/05B demonstrate real shared lifecycle code, not by pre-creating a generic finisher framework.
...
- TODO-05A follow-up recommendations are deliberately conditional, not current implementation permission: (1) `TODO-05A1` may address bidirectional execution lock-in and lethal-hit Recovery only if later PIE or product acceptance proves the truncation matters; (2) `TODO-05B` must first settle execution-range versus Motion-Warp range semantics and may extract a shared execution base only after measured duplication; (3) `TODO-07B8` may adopt the existing RateWindow lifecycle for Enemy Stance Break/reaction Montages after real enemy assets exist; (4) `TODO-07B9` may add execution-only Hit-Stop/Camera Shake/audio through the typed feedback profiles after lifecycle semantics stabilize. These follow-ups are not hard prerequisites for the completed 05A stage or the next 05B source plan.
...
- TODO-05A source implementation, `PolyQuest.Combat.FrontExecution` Automation, user compile/Editor readback/Scene01 PIE, and Main Fresh Review are complete with no P0-P2 blocker. The v1 contract intentionally remains ordinary-attack cancelable; any lock-in/Recovery change belongs to conditional `TODO-05A1`.
